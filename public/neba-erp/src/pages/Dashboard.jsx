...
function TickerBar({ products, totalRev, topProduct, t }) {
  const items = [
    ...
    { icon: '□', label: t('dashboard.tickerProducts'),   val: `${products.length} Products` },
                ...
                <div key={p.id} style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'7px 9px', borderRadius:8, marginBottom:5,
                  ...
                  <div style={{ display:'flex', alignItems:'center' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:10.5, marginTop:1, fontWeight:300, opacity:.75, color: isOut ? 'var(--red-text)' : 'var(--yellow-text)' }}>{t('products.currentStock')}: {p.stock}</div>
                    </div>
                  </div>
